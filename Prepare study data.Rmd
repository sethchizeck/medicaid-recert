...
load(file = paste0(filepath, "/Data/Medicaid enrollment data/sample_enrollment_dates_adults.rda"))
...
# Now see if the person was enrolled in Medicaid at any point between the flagged "non-binding" deadline and the most recent prior binding deadline. If not, then we can drop the flagged deadline. If so, then keep it because it was still binding in the sense that it was the person's known upcoming deadline.
  ...
  select(mci_uniq_id, child, recert_deadline)
...
random_sample <- sample_recert_deadlines %>% 
  distinct(mci_uniq_id) %>%
  slice_sample(n = 10)

sample_enrollment_dates <- sample_enrollment_dates %>%
  inner_join(random_sample, by = c('mci_uniq_id'))
...
# Create the grid of dates
  ...
  group_by(mci_uniq_id, person_deadline_id, child, recert_deadline) %>%
...
#----------------------------------------------------------------------------
# Combine the enrollment day panel with the +/- 372 day window of dates around each recert deadline
...
# Roll up the data by week
  ...
  group_by(mci_uniq_id, person_deadline_id, child, recert_deadline, tau_week) %>%
  summarise(week_start_date = min(date),
            medicaid_enrolled = ifelse(sum(medicaid_enrolled) > 0, 1, 0)) %>%
  ungroup() %>%
  filter(abs(tau_week) <= 52) %>%
  
# Create a variable for the number of days between 1/1/2020 and the beginning of the week

  mutate(calendar_week = as.numeric(floor(difftime(week_start_date, as.Date('2020-01-01'), units = "weeks"))) + 1) %>%
