...
load(file = paste0(filepath, "medicaid_enrollment_panel_weekly.rda"))

#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
# Test for parallel trends in pre-treatment period

pre_trend_data <- medicaid_enrollment_panel_weekly %>% 
  filter(tau_weeks < 0)

pre_trend_test <- feols(medicaid_enrolled ~ tau_weeks | person_deadline_id + calendar_week,
                        data = pre_trend_data,
                        cluster = "mci_uniq_id")
...
event_study <- feols(medicaid_enrolled ~ i(tau_weeks, ref = -1) | person_deadline_id + calendar_week,
                     data = medicaid_enrollment_panel_weekly,
                     cluster = "mci_uniq_id")

summary(event_study)

# Alternative: Use binned endpoints for cleaner estimation

event_study_binned <- feols(medicaid_enrolled ~ i(tau_weeks, ref = -1) | person_deadline_id + calendar_week,
                           data = medicaid_enrollment_panel_weekly,
                           cluster = "mci_uniq_id")

summary(event_study_binned)
...
event_coefs <- summary(event_study)$coefficients

event_coefs_df <- data.frame(weeks_since = as.numeric(gsub("weeks_since_deadline::", "", rownames(event_coefs))),
                              coef = event_coefs[, "Estimate"],
                              se = event_coefs[, "Std. Error"]) %>%
  mutate(ci_lower = coef - 1.96 * se,
        ci_upper = coef + 1.96 * se) %>%
  
# Add reference point (week -1 = 0)
  
  bind_rows(data.frame(weeks_since = -1, coef = 0, se = 0, ci_lower = 0, ci_upper = 0))

# Create event study plot

ggplot(event_coefs_df, aes(x = weeks_since, y = coef)) +
  geom_point() +
  geom_line() +
  geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.3) +
  geom_vline(xintercept = -0.5, linetype = "dashed", color = "red") +
  geom_hline(yintercept = 0, linetype = "solid", color = "gray") +
  labs(title = "Event Study: Effect of Recertification Deadline on Medicaid Enrollment",
    x = "Weeks Since Recertification Deadline",
    y = "Treatment Effect (Percentage Points)",
    caption = "95% confidence intervals shown. Week -1 normalized to zero.") +
  theme_minimal()
```
...
dt <- medicaid_enrollment_panel_weekly %>%
  group_by(tau_weeks) %>%
