...
#----------------------------------------------------------------------------
# Create a data frame with one obs per person per day that they were enrolled in Medicaid between 4/1/2023 and 6/30/2024

# Keep only the recert deadlines from between 4/1/2023 and 6/30/2024. This date range will give us at least 365 days of outcome data for every person (since our enrollment data only goes through 6/30/2025 right now)

sample_recert_deadlines <- sample_recert_deadlines %>%
  filter(recert_deadline >= as.Date('2023-04-01') & recert_deadline <= as.Date('2024-06-30')) 
...
#----------------------------------------------------------------------------
# Join with a grid of each date within +/- 365 days of the person's recert deadline
...
dt <- dt %>%
  right_join(recert_date_window_365, by = c('ma_receipt_num', 'date')) %>%
